...
BOOKナビゲーター、アセット、インスペクター、タイムラインには、ARIAと安定した `data-tobidas-*` 識別子があります。
作品ID、選択対象、アクティブな見開き、プレビュー位置は標準ワークスペースの属性から確認できます。

Canvas座標を推定しない配置には、部品プリセット見出しの「詳細配置」を使います。
左右ページ、プリセット、取り込み済みアセット、正規化座標を指定して配置できます。
...
リポジトリには、プロンプトから作品の設計、素材準備、標準ビルダーとWebMCPによる構築、検証までを支援する [`tobidas-create` スキル](./.agents/skills/tobidas-create/SKILL.md) も同梱しています。スキル対応のCodexやエージェント環境から利用できます。
