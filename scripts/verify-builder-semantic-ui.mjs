...
try {
  ...
  const rightResizeHandle = page.getByRole('separator', { name: 'インスペクターの高さ', exact: true })
  await rightResizeHandle.waitFor()
  const inspectorPane = page.locator('#right-inspector-mobile-pane')
  const inspectorBefore = await inspectorPane.boundingBox()
  const handleBox = await rightResizeHandle.boundingBox()
  if (!inspectorBefore || !handleBox) throw new Error('右側ペインのリサイズ境界を取得できません')
  await page.mouse.move(handleBox.x + handleBox.width / 2, handleBox.y + handleBox.height / 2)
  await page.mouse.down()
  await page.mouse.move(handleBox.x + handleBox.width / 2, handleBox.y + handleBox.height / 2 + 32)
  await page.mouse.up()
  const inspectorAfter = await inspectorPane.boundingBox()
  if (!inspectorAfter || inspectorAfter.height <= inspectorBefore.height) throw new Error('右側のインスペクター境界をドラッグできません')
  const guideFields = authoringGuide.locator('textarea')
  if (await guideFields.count() !== 17) throw new Error(`制作ガイドの入力項目数が不正です: ${await guideFields.count()}`)
  ...
  await guideField.blur()
  const languagePicker = page.getByLabel('表示言語')
  ...
  const mobileInspectorToggle = mobile.getByRole('button', { name: 'インスペクター', exact: true })
  const mobileGuideToggle = mobile.getByRole('button', { name: '制作ガイド', exact: true })
  await mobileInspectorToggle.waitFor()
  await mobileGuideToggle.waitFor()
  if (await mobileInspectorToggle.getAttribute('aria-expanded') !== 'false'
    || await mobileGuideToggle.getAttribute('aria-expanded') !== 'false') {
    throw new Error('モバイルの右側ペインが既定で収納されていません')
  }
