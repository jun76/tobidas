...
7. 地面、空間背景、立体パーツ、テキスト、パーティクルを見開きごとに配置し、名前、位置、寸法、レイヤー、表示状態を更新する。本文は所有ページへ固定印字し、テキスト要素へ演出トラックを追加しない。
8. AIモードでは表紙、カメラ、光源、常時モーション、オーサードタイムライン、音声キューを標準モードと同じ作品データへ設定できる。AIモードで対象、プロパティ、時刻、値を明示してキーを追加し、ビューポート下のタイムラインで再生、スクラブ、補間、時刻変更、削除を確認する。
