 ...
 * 離れず、紙が視線に対して寝るほど射影が縮んで、寝ている部品が軒並み紙へ埋もれた。
 * 実測では二面角 45° で紙面の部品が全滅し、起き上がる位相まで一つも見えなくなる。
 *
 * `layerDepthBias` の polygonOffset を当てにしてはいけない。実装依存で丸ごと無視される
 * ことがあり (headless Chromium の WebKit WebGL では -5000 単位でも動かなかった)、
 ...
 * 一瞬だけ現れるのがこれ (実測: layer 20 のランタンが 0.072 浮き、紙の厚み 0.015 を
 * 越えて前の面へ抜けた)。layer 100 まで積んでも 0.007 で、既定の 0.015 の半分に収まる。
 */
const SURFACE_Y = 0.006
const LAYER_LIFT = 0.00001
/** 相似縮小が働き始める展開係数。これ以上では厳密に恒等 (docs/006 §6) */
...
export function evaluateStow(item: StowItem, tRaw: number, motion: MotionDelta): StowPose {
  ...
  // 閉じ際だけ働く相似縮小 (docs/006 §6)。展開係数と結び付け、
 ...
 * ページの傾きに乗って本の外へ飛び出す (実測: 半径2.1の子が紙面から1.2上がり、
 * 世界座標で片面幅の外側へ出た)。だから隔たりのほうを畳む必要がある。
 *
 ...
 *
 * 隔たりを畳まずに「開姿勢での足跡へ降ろす」やり方も試したが、これは成り立たない。
...
/**
 * 背をまたぐ一枚パネルの閉形式評価 (docs/006 §5)。剛体V-foldの球面リンク。
 *
...
/** 窓関数: t=0とt=1で必ず0。装飾は端点を崩せない (docs/006 §9) */
