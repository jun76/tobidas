...
/**
 * 外部ファイルの音声は Web Audio へ通さず HTMLAudioElement のまま鳴らす (docs/009 §4)。
 *
...
export class AudioPlayback {
   ...
   * 状態になる (実測: ホイールで転がして始めると音源が2本になった)。
   *
...
/** 効果音の音量。キューにもトラックにも持たせない (docs/008 §6.1) */
 ...
 *
 * 発火は一度きりで、同じ音が重なることを許す (docs/008 §6.1)。前の発音を
...
export class AudioBank {
  ...
  /**
   * 効果音を止める / 戻す。再生が止まっている間は鳴らさない (docs/008 §7.2)。
   *
